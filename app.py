...
if uploaded_file is not None:
    ...
    for section in lot_sections[1:]:
        ...
        for part in parts:
            try:
                rough_wt = float(part[1])
                exp_carat = float(part[2])
                rough_to_polish_pct = float(part[3]) / 100
                ...
                print(f"  - Data extracted: {rough_wt}, {exp_carat}, {rough_to_polish_pct}")
        ...
        all_data.append(["", "", "", "", "", ""])
        for exp_carat_value in lot_exp_carat:
            print(f"  - code no2= {code}")
            print(f"  - seen codes= {seen_codes}")
            if code not in seen_codes:  # Check if code has been seen before
                seen_codes.add(code)
                all_data.append([sr_no, code, round(lot_rough_wt,2), "", round(exp_carat_value,2), round(rough_to_polish_pct,2)])
            else:
                all_data.append(["", "", "", "", round(exp_carat_value,2), ""])
